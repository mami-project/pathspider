...
# 	'''
# 	Takes a domain name and resolves it to up to one IPv4 up to one IPv6
# 	address.
#
# 	For deciding whether a resolution was successful (and therefore no
# 	fallback must be used for www vs. no-www) only A records are
# 	considered.
#
# 	:param str fields: A list of rank and domain name to be resolved. The
# 	rank will not be interpreted, but simply passed on to the output.
# 	:returns: A list of IP addresses. The list may be empty if errors are
# 	encountered during the resolution.
# 	'''
# 	sleep(SLEEP)
# 	rank = fields[0]
# 	domain = fields[1]
#
# 	# NOTE: www.com or www.co.uk would be incorrectly handled by checking
# 	for a leading "www." first. Alexa's list generally omits the almost
# 	ubiquitous "www.", but not always: www.uk.com is a counter-example.
# 	wdomain = domain
# 	#if domain[:4] != 'www.':
# 		#wdomain = 'www.' + domain
# 	wdomain = 'www.' + domain
...
#
# 	# To make the code shorter, at this stage ``domain`` might actually
# 	have been assigned the value of ``wdomain``, and so, may have an added
# 	'www.'. It depends on the WWW mode.
#
